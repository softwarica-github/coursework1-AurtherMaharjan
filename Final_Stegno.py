...
class Stegno:
    ...
    art ='''¯\_(ツ)_/¯'''
    art2 ='''This is just for educational purpose'''
    output_image_size = 0

    def main(self,root):
        root.title('SimpleSteganography')
        root.geometry('700x500')
        root.resizable(width =True, height=True)
        f = Frame(root)

        title = Label(f,text='SimpleSteganography')
    ...
    def frame1_decode(self,f):
        f.destroy()
        d_f2 = Frame(root)
        label_art = Label(d_f2, text='Please')
    ...
    def frame1_encode(self,f):
        f.destroy()
        f2 = Frame(root)
        label_art = Label(f2, text='''Please''')
